...
malder_out <- paste0("~/R/Copy/Rprojects/AfricaPOPGEN/manuscript/f3tables/AllPopsMalderFinalALL.txt")
...
for(analy in analys)
{
    ...
    for(i in (1:nrow(poplist)))
    {
        ...
        {
            tt <- all_res2[all_res2[,"pop1"]==pop1,]
...
write.table(tmp.tab,malder_out,quote=F,col.names=T,row.names=F)
...
print(tmp.tab,include.rownames=F,floating=F,include.colnames=T,
      ...
      file=paste0("~/R/Copy/Rprojects/AfricaPOPGEN/manuscript/f3tables/AllPopsMalderFinalALL.tex"),
