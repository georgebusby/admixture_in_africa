...
for(pop1 in pops)
{
    for(pop2 in pops)
    {
        if(pop1 != pop2 & pop1 != "CEU")
        {
            ...
            out_file <- paste0(out_dir,sim,out_suff,".chunklengths.out")
            ...
            for(gen in gens)
            {
                for(prop in props)
                {
                    ...
                    for(chr in 1:22)
                    {
                        ...
                        sim_file <- paste0(length_dir,sim_pop,out_suff,"Chrom",chr,".chunklengths.out")
            ...
            if(!is.null(xs))
            {
                ...
                {
                    lengths <- paste0(cp_dir,
                                      "MalariaGen23EthnicGroups1KGSouthAfricaNoAmericaFinalAllDons.chunklengths.out")
                    x <- read.table(lengths,header=T,row.names=1)
                    ## REMOVE both donor pops COLUMNS AS THE SIMS WERENT' PAINTED WITH THESE POPS
                    x <- x[,!colnames(x)%in%c(pop1,pop2)]
                    x <- rbind(x,xs[,colnames(x)])
                    x <- x/rowSums(x)
                    x <- cbind(rownames(x),x)
                    colnames(x)[1] <- "Recipient"
