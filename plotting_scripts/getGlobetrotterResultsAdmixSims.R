...
in_dir <- "/mnt/kwiat/well/human/george/admix_sims/globetrotter/lengths/"
res_dir <- "/mnt/kwiat/well/human/george/admix_sims/globetrotter/props/"
dates_dir <- "/mnt/kwiat/well/human/george/admix_sims/globetrotter/dates/"
param_dir <- "/mnt/kwiat/well/human/george/admix_sims/globetrotter/paramfiles/"
analysis <- ""
c_suffix <- ".globetrotter.main_curves.txt"
p_suffix <- ".globetrotter.main.txt"
a_suffix <- ".globetrotter.ancient.txt"
...
sim_src1 <- sapply(sim_srcs,function(x) as.character(unlist(sim_srcs[x])[1]))
...
for(pop in sim_pops)
{
    ...
    pop1 <- pop
    sim_srcs <- as.character(sim_tab[pop,"sim_srcs"])
    #reg <- as.character(popkey$RegionM[popkey$Ethnic_Group==sim_pop1])
    nlength <- read.table(paste0(in_dir,analysis,sim_srcs,".chunklengths.out"),header=T,row.names=1)
    nlength <- rowsAsMapClusts(final_clusts2,nlength,stat=mean)
    predmat <- nlength/rowSums(nlength)
    donors <- gsub("-",".",popplot)
    
    for(a in c("main","ancient"))
    {
        infile <- paste0(res_dir,pop1,"nolocal.globetrotter.",a,".txt")
        infile_null <- paste0(res_dir,pop1,"nolocal.globetrotter.null.txt")
        infile_dates <- paste0(dates_dir,pop1,"nolocal.globetrotter.",a,".dates.bt")
        infile_2dates <- paste0(dates_dir,pop1,"nolocal.globetrotter.",a,".2dates.bt")
        infile_dates_null <- paste0(dates_dir,pop1,"nolocal.globetrotter.null.dates.bt")
        if(file.exists(infile))
        {
            ...
            if(nrow(ld)>0) dateboots <- rbind(dateboots,cbind(pop,a,ld))
            if(nrow(ld2)>0) date2boots <- rbind(date2boots,cbind(pop,a,ld2))
            ## ADD TO THE MAIN RESULTS TABLE
            resmat <- rbind(resmat,c(pop,ll[[1]],a))
            
            ## GET THE RESULTS OF THE NULL RUN
            if(file.exists(infile_null))
            {
                lln <- getGlobetrotter(infile=infile_null)
...
for(i in restypes)
{
    ...
    if(nrow(ii) == 1)
    {
        ii[numericcols] <- sapply(signif(sapply(ii[numericcols],as.numeric),3),as.character)
        ii <- convert.factors.to.strings.in.dataframe(ii)
        ii <- ii[order(ii$bestmatch.event1.source1,ii$gen.1date,decreasing=T),]
    }
...
for (analy in c("main","ancient"))
{
    sims2plot <- as.character(sim_tab[finaltable$Cluster,"sim_src2"])[finaltable$Analysis == analy]
    xs <- as.numeric(as.character(sim_tab[finaltable$Cluster,"sim_gens"]))[finaltable$Analysis == analy]
    ys <- as.numeric(finaltable$Date)[finaltable$Analysis == analy]
    ys2 <- as.numeric(finaltable$Date.2a)[finaltable$Analysis == analy]
    ys3 <- as.numeric(finaltable$Date2b)[finaltable$Analysis == analy]
    
    plot(0,0,xlim=c(0,750),ylim=c(0,750),type="n",
         xlab="Simulated Date",ylab="GLOBETROTTER date",
         main=analy)
    abline(a=0,b=1,lwd=2)
    points(xs,ys,pch=20,col="black")
    #points(xs,ys2,pch=20,col="red")
    points(xs,ys3,pch=20,col="blue")
    
    for(i in unique(xs))
    {
        points(i,median(subset(ys,xs==i)),pch=15,cex=2,col="black")
        points(i,median(subset(ys3,xs==i)),pch=15,cex=2,col="blue")
    }
}
...
write.csv(res.tabA,file=paste0(main_dir,"popgen/figures/AfricaGlobetrotterSimResults.csv"),quote=F,row.names=F)
...
res.ord <- rt1[,"best.source1"]
res.ord[rt1$Result=="2D"] <- rt1$best.source1.date1[rt1$Result=="2D"]
res.ord.levs <- popplot
res.ord <- factor(res.ord,levels=res.ord.levs)
rt1 <- rt1[order(res.ord,as.numeric(rt1[,"date.1D"])),]
res.ord <- c(res.ord,rep("XXX",nrow(rt2)))
res.tabB <- rbind(rt1,rt2)
## PULL OUT THE NULL RESUTLS ##
res.tabB <- res.tabB[grep("main.null",res.tabB[,2],invert=T),]
#############################################################
## THE FINAL MAIN RESULTS TABLE
write.csv(res.tabB,file=paste0(main_dir,"popgen/figures/AfricaGTsimsBESTtable.csv"),quote=F)
# #############################################################
...
res.tabB <- res.tabB[,res.tabcols[res.tabcols%in%colnames(res.tabB)]]
...
write.table(final.res2plot,file=paste0(main_dir,"popgen/data/MalariaGenGlobetrotterSims2plot.txt"))
write.table(admixturesources2,paste0(main_dir,"popgen/data/MalariaGenGlobetrotterSimsAdmixtureSources2.txt"))
write.table(admixturesources3,paste0(main_dir,"popgen/data/MalariaGenGlobetrotterSimsAdmixtureSources3.txt"))
write.table(admixturesources4,paste0(main_dir,"popgen/data/MalariaGenGlobetrotterSimsAdmixtureSources4.txt"))
write.table(dateboots,paste0(main_dir,"popgen/data/MalariaGenGlobetrotterSimsOneDateBootstraps.txt"))
write.table(date2boots,paste0(main_dir,"popgen/data/MalariaGenGlobetrotterSimsTwoDateBootstraps.txt"))
